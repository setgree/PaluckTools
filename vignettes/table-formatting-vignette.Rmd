...
- `run_subset_meta_analysis()`: Run meta-analysis on data subsets
- `run_meta_regression()`: Compare moderator levels via meta-regression
- `process_group()`: Create complete moderator tables with one function
...
The `extract_model_results()` function runs `robumeta::robu` and extracts key statistics in a clean format: number of studies and estimates, effect size (Delta), standard error, confidence interval, p-value, and tau (between-study heterogeneity).
...
The output includes:
- `Moderator`: The level name (RCT, Quasi-experimental, etc.)
...
- `p_val_ref`: P-value comparing this level to the reference level (RCT)

Notice "ref" for RCT's `p_val_ref`—it's the reference level, so other levels are compared to it.
...
behavior_table <- sv_data |>
  process_group(
    ...
    ref_level = "Attitudes",
    order_levels = c("Attitudes", "Perpetration", "Victimization"),
    include_tau = FALSE
  )
...
rct_only <- sv_data |>
  run_subset_meta_analysis(
    group_var = "study_design",
    level = "RCT",
    approach_name = "Randomized Controlled Trials",
    include_tau = TRUE
  )
