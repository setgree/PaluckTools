...
- **Run many specifications** → `tidy_lm()`
- **Cluster-robust SEs for one model** → `robust_se()`
- **Create stargazer tables from tidy_lm** → `star_ready()`
...
For complete function documentation, see `?process_group`, `?run_subset_meta_analysis`, `?extract_model_results`, `?table_prep`, `?star_ready`.
